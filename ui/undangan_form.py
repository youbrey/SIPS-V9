...
class UndanganFormMixin:
    ...
    def show_undangan_akd(self):
        self.current_view = "undangan_akd"
        self.right_frame.grid_remove()
        self.setup_undangan_akd_form()
    ...
    # ------------------------------------------------------------------
    # FORM: SURAT UNDANGAN RAPAT AKD (placeholder, silakan kembangkan)
    # ------------------------------------------------------------------
    def setup_undangan_akd_form(self):
        for widget in self.middle_frame.winfo_children():
            widget.destroy()
        title = ctk.CTkLabel(self.middle_frame, text="📋 Surat Undangan Rapat AKD",
                              font=("Arial", 15, "bold"), text_color="#8B5CF6")
        title.pack(fill="x", padx=10, pady=(10, 10))
        info = ctk.CTkLabel(
            self.middle_frame,
            text="Form Rapat AKD belum diimplementasikan.\n\n"
                 "Tambahkan field & logikanya di:\n"
                 "- ui/undangan_form.py  (method setup_undangan_akd_form)\n"
                 "- letters/undangan.py  (fungsi buat_undangan_akd)",
